...
var createError =  function(obj) {
    ...
    //create error will process an error object and handle it based on its configuration
  ...
  jQuery("div .ps-error-" + obj.id).removeClass("ps-error-" + obj.id).removeAttr("data-error-" + obj.id);
  ...
  _.each(obj.field, function(item) {
    ...
    if (jQuery("." + item + " input  , ." + item + " textarea ").filter(function() {
        if (!obj.hasOwnProperty("test")) {
          //true here allows for informational messages
          return true
        }
        return obj.test(this)  /*run the script on the definition */
    }).parent( "." + item  ).addClass("ps-error-" + obj.id).attr("data-error" ,"wrapper-" + obj.id) /* style the container */
      .prepend("<div data-error='text-" + obj.id + "'  class='ps-error-text-" + obj.id + "'>" + obj.desc + "</div>").length > 0) {
      found = true;
    }
  })
