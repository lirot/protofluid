jwt.jwtComponent = (function() {
  ...
  // the object is the invoice transaction document. the object is mostly organized
  // in a heaer line format, but does have a group section that aggreages lines with
  // similar project and purchase orders looking at the page on line shows the
  // organization most clearly.  Besides the transaction data this oject stores
  // error data related to the trasaction used later for display and edit logic
  // the object also stores user related data for siilar purpose.  The script that
  ...
  // heavily dependent on the input element naming convention established in the
  // peopletools page desinger.  There each input element is named in a fashion
  // to allow for easier parsing.  For example, page elements are given element
  // id starting with XX.  Header and line elements are XX_HSDR and XX_LIN
  // respectively.  This aides greatly in crating the resulting javascript object
  // finally.  The javascript element is passsed to downstrad to a html templating
  // processor and vinally to a callback script allowing for more general purpose
  ...
  var _parse = function(config, response, InstanceID) {
    ...
    var wLogin = response.indexOf("<TITLE>Oracle | PeopleSoft Enterprise 8 Sign-in</TITLE>");

    (wLogin > 0) ? window.location.href = "http://" + location.host + "/psp/" + url_xx[4] + "/EMPLOYEE/ERP/?cmd=login": null;

    var wOpen = response.indexOf("window.open(");

    if (wOpen < response.indexOf("<form") && wOpen > 0) {
      var nWindow = response.substring(wOpen, response.indexOf(");", wOpen) + 2);
      eval(nWindow);
    }
    ...
    if (wAlert < response.indexOf("<form") && wAlert > 0) {
      var nAlert = response.substring(wAlert, response.indexOf(");", wAlert) + 2);
      eval(nAlert);
    ...
    var jqForm = jQuery("<form class='xxForm' ></form>").attr('action', config.formAction).attr('name', formName);

    jQInvComp.find("input[type='hidden']").filter(function() {
    ...
    if (jQInvComp.find('#XX_HDR_SS').val() == 'IMG' || window.unlock) {
      ...
      if (jQInvComp.find('#XX_HDR_SS').val() == 'IMG' || jQInvComp.find('#XX_HDR_SS').val() == 'NEW') {
      ...
      if (  elem.attr('name') == "XX_HDR_AWE" ) {
        ...
        if ( elem.attr('name') == "XX_HDR_VT" ){
            ...
            _.each( elem.find("li") , function(obj){
                ...
                if (  retObj ) {
                   console.log( jQuery(obj).attr('id') + " : " +  jQuery(obj).html() );
                   ...
                   console.log("server error message not mapped!" + jQuery(obj).attr('id') + jQuery(obj).html()  );
               }
               }
            )
        }
    ...
    _.each(jwt.lines, function(obj) {
      ...
      var sectionHeader = {};
      var temp =  { key : obj.key , pi : obj.XX_LIN_PI , po : obj.XX_LIN_PO , section_ce : obj.XX_LIN_CE }
    ...
    jwt.invoice.workflowLines = ( jwt.workflow.hasOwnProperty( 'rs_xx_289_awe_m_vw' ) ) ? jwt.workflow.rs_xx_289_awe_m_vw.row_xx_289_awe_m_vw : "";

    jwt.invoice.header = jwt.header;

    jwt.invoice.header.Approverdisabled = jwt.user.isApprover ?  "disabled" : "";
    ...
    _.each(keys, function(keyObj, index) {
        ...
        _.each(jwt.lines, function(obj2, index2) {
            (obj2.key == keyObj.key) ? jwt.invoice.sections[index]["lines"].push(obj2): null;
        })
    })

    //run the can view and can edit function to determine display settings for the route buttons
    //and fields
  ...
  var xxAjax = function(config, instanceid, theform, name) {
    ...
    // jwt.jwtWorkList.sActivePage = "jwt." + config.Definition;
    ...
    if (!instanceid) {
      ...
      var jqXHRoptions = {
        type: "POST",
        url: "http://" + location.host + "/psc/" + url_xx[4] + config.formAction,
        contentType: "application/x-www-form-urlencoded"
      };
      var input ;
      //grab all inputs in memory and push them on the form being sent over the AJAX call
      jQuery(" *[ name^='XX']" ).each( function( index){
              input = jQuery(this).val() ? jQuery(this).val()  : " ";
              jQuery("<input name='" + jQuery(this).attr('name') + "'  value='" + input + "' >").appendTo( jwt.form);
      })
     ...
     if (config.saveMethods.indexOf( name ) )
      ...
      jwt.isDirty = false;
      //gets off the worklists

      jqXHRoptions = {
        async: true,
        type: "POST",
        url: "http://" + location.host + "/psc/" + url_xx[4] + config.url + instanceid,
        contentType: "application/x-www-form-urlencoded"
      };
    }
