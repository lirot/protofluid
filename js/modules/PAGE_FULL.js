    ...
    return {
                        ...
                        jQuery(".group[data-key='" + obj.SectionHeader.key + "']").find(".xx-link-pi-setup , .xx-link-po-modify, .xx-link-pj , .xx-link-estimate")
                            .click(
                                function(event) {
                                    ...
                                    window.open("http://" + location.host + "/psc/" + url_jwt[4] + "_newwin/EMPLOYEE/ERP/s/WEBLIB_XX_NAV.WEBLIB_FUNCTION.FieldFormula.iScript_" + jQuery(this).attr("data-script-id") + urlqryString, '', '');
                                });
        ...
        callback: function(config) {
            ...
            var foundlines = _.filter(jwt.lines, function(obj) {
                return obj.XX_LIN_AO == "N" || obj.XX_LIN_AF == "Y" || obj.XX_LIN_AP || obj.XX_LIN_AI
            });
            ...
            _.each(foundlines, function(obj) {
                ...
                if (jwt.user.isSAS) {
                    if (obj.XX_LIN_AF == "Y" ||
                        jQuery(".line2[data-row-id='" + rowID + "']").find(".rf-lin-ai > input,  .rf-lin-ap > input").filter(function() {
                            return this.value
                        }).length > 0) {
                        jQuery(".line2[data-row-id='" + rowID + "']")
