...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		jwt.jwtData.init('xx_proj_attrtbl'
            ...
            jQuery.when.apply(undefined, promises).promise().done(
			  ...
			  .html(outHTML);
                      ...
                      // the system will make a call back to a
		      // method on the data object xx_po_rpt to format and
		      ...
		      var key = obj.SectionHeader.bu + obj.SectionHeader.pt;
			     ...
			     + "']").attr('data-pt-bu', key);
                      ...
                      // the remaining code in the function is to
		      // direct the clicks on the links
   ...
   controller: function(elem) {
	    ...
	    // handles updateing form elements
   ...
   callbackAlternate: function(config) {
	...
	//controls and there values set
        ...
        // if there is an error bring the user to the top of the
	// page using a jquery animation
            jQuery(".messages").on("hasError", function() {
                jQuery("#component-data").animate({
                  //  scrollTop: 0
                }, "fast");
            });

        // running edits here will display all the server errors
	// using the error conroller logic
            jwt.routes.run_Edits('jwtComponentConfigFull', jwt.invoice);

        // this method call will both display buttons and
	// bind the clicks to those buttons based on the routes
	// that are configured
        jwt.routes.xx_bind_buttons();

        // there is functionality to configure fields to drive display
	// and edit properties
	...
	var fieldList = _.filter(jwt.fields, function(obj) {
	    ...
	    // in the ma field on the first line
        ...
        jQuery('.rf-lin-lq > input , .rf-lin-up > input')
	     .on('change', function() {
            ...
            _.each(foundlines, function(obj) {
                //for each row that has a amount only NO or an asset
